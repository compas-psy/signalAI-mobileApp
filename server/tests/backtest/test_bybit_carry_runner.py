...
def test_resolve_carry_facts_never_uses_future_settlement_to_infer_first_interval() -> None:
    ...
    # The first funding print alone cannot establish the historical interval.
    # The second print may establish it using only information known then, so
    # eight honest PIT facts exist rather than nine look-ahead facts.
    assert len(facts) == 8
    assert facts[0].tradable_at == start + timedelta(hours=16)
