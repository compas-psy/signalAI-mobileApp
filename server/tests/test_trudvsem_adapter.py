"""Контракт официального API «Работа России» для HIRING."""
...
def test_пустая_зарплата_не_становится_нулём():
    second = trudvsem.parse(fetched(PAYLOAD))[1]
...
def test_курсор_двигается_по_официальному_времени_изменения():
    cursor = trudvsem.checkpoint(trudvsem.parse(fetched(PAYLOAD)))
...
def test_пагинация_учитывает_total_и_предел_источника():
    pages = trudvsem.remaining_pages(fetched({**PAYLOAD, "meta": {"total": "205"}}))
    assert len(pages) == 2
    assert "offset=1" in pages[0].url
    assert "offset=2" in pages[1].url

    capped = trudvsem.remaining_pages(fetched({**PAYLOAD, "meta": {"total": "50000"}}))
    assert len(capped) == 99
    assert "offset=99" in capped[-1].url


def test_доступность_считается_от_изменения_а_не_от_даты_роли():
    when = trudvsem.availability(trudvsem.parse(fetched(PAYLOAD))[0], first_seen_at=NOW)
