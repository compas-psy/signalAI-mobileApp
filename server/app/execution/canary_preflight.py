...
provider.  It re-checks immutable non-secret policy facts against the currently
deployed runtime, credential generation and exact server evidence refs.
Governance/owner step-up remains unresolved, so even a fully clean result is
never eligible for Canary yet.
"""
...
from .canary_evidence import evaluate_canary_evidence_bindings
...
_LEGACY_EVIDENCE_BLOCKER = "CANARY_EVIDENCE_BINDING_NOT_IMPLEMENTED"
...
def _authoritative_ref_shape(evidence_refs: object) -> bool:
    """Distinguish exact snapshot refs from old placeholder policy strings.

    Old proposed-policy fixtures used arbitrary labels.  They remain permanently
    blocked by the legacy blocker rather than being silently reinterpreted.  New
    policies use seven UUID refs and must pass the authoritative resolver.
    """
    if not isinstance(evidence_refs, Mapping) or len(evidence_refs) != 7:
        return False
    try:
...
def evaluate_canary_preflight(
    ...
    *,
    ...
    runtime, credential or evidence checks.  The function never changes execution
    mode, never decrypts a key and never constructs a provider transport.
    """
    ...
    if not _authoritative_ref_shape(evidence_refs):
        # Never reinterpret old arbitrary labels as durable authority. They stay
        # blocked until a new immutable policy snapshot names exact UUID facts.
        return CanaryPreflightResult(
            eligible_for_canary=False,
            structural_checks_passed=True,
            blockers=_GOVERNANCE_BLOCKERS + (_LEGACY_EVIDENCE_BLOCKER,),
        )

    evidence = evaluate_canary_evidence_bindings(
        db,
    ...
    # Evidence is now structurally proven, but it is still not authorization.
    # ADR acceptance and a separate fresh owner step-up remain mandatory.
    return CanaryPreflightResult(
        eligible_for_canary=False,
        structural_checks_passed=True,
