...
class PaperTrade(UuidPk, Base):
    ...
    __table_args__ = (
        # Одна живая сделка на инструмент.
        #
        # Раньше индекс стоял по идее — с доводом «зависшая сделка не должна
        # блокировать новые идеи по тому же инструменту». Довод верный,
        # вывод неверный: скан находит тот же сетап каждые пятнадцать минут,
        # и каждая находка становилась отдельной сделкой. У владельца в
        # «В работе» оказалось десять позиций по HYPEUSDT с входами
        # 52,2860 … 52,2950 — одна сделка, пересчитанная десять раз.
        #
        # Быть одновременно в десяти позициях по одному активу — не
        # диверсификация, а десятикратный риск на нём. Зависание лечится
        # календарным протуханием и починенной сверкой, а не отсутствием
        # дедупа.
        Index(
