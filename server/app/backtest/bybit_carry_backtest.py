...
def calculate_carry_outcome(
    *,
    ...
    settlements = tuple(
        ...
        if entry_at < item.settled_at <= exit_at and item.tradable_at <= exit_at
    )
...
def replay_carry_oos(
    *,
    ...
    ordered by ``tradable_at``. The strategy sees only each fact's funding
    ...
    for index, current in enumerate(ordered):
        if current.tradable_at < next_available_at.get(
            current.instrument_id, current.tradable_at
        ):
            continue
        ...
        exit_fact = next(
            (
                ...
                if item.instrument_id == current.instrument_id
                and item.tradable_at >= target_exit
            ),
            None,
        )
...
def _report(outcomes: tuple[CarryOutcome, ...], gate: CarryReplayGate) -> CarryReplayReport:
    ...
    if not values:
        ...
        profit_factor = (
            ...
            else (positive / negative if negative > 0 else Decimal(0))
        )
