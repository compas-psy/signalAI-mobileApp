...
def resolve_carry_facts(dataset: ResolvedDataset) -> tuple[CryptoCarryMarketFacts, ...]:
    """Build deterministic PIT facts from the immutable snapshot rows."""
    ...
    for mark in marks:
        while index_cursor < len(indexes) and indexes[index_cursor].tradable_at <= mark.tradable_at:
            latest_index = indexes[index_cursor]
            index_cursor += 1
        while funding_cursor < len(funding_history) and funding_history[funding_cursor].tradable_at <= mark.tradable_at:
        ...
        interval_minutes = int((latest.settled_at - previous.settled_at).total_seconds() // 60)
        ...
        facts.append(
            CryptoCarryMarketFacts(
                ...
                observed_at=max(mark.tradable_at, latest_index.tradable_at, latest.tradable_at),
...
def run_bybit_carry_backtest(
    ...
    *,
    ...
    """Run and persist the carry OOS evidence for one immutable snapshot."""
    ...
    report = replay_carry_oos(
        ...
        hedge_carry_bps_per_interval=Decimal(str(raw["hedge_carry_bps_per_interval"])),
        funding_uncertainty_bps_per_interval=Decimal(str(raw["funding_uncertainty_bps_per_interval"])),
    ...
    payload = {
        ...
        "cost_model": {
            ...
            "hedge_carry_bps_per_interval": str(raw["hedge_carry_bps_per_interval"]),
            "funding_uncertainty_bps_per_interval": str(raw["funding_uncertainty_bps_per_interval"]),
            "funding_uncertainty_used_in_realized_outcome": False,
        },
    }
    run = BacktestRun(
        label=f"bybit-carry-backtest:crypto_carry_v1:{dataset.snapshot_id[:12]}",
...
def _blocked(
    ...
    *,
    ...
    run = BacktestRun(
        ...
        report_json={"metric_space": "CARRY_BPS", "reason": reason, "dataset": dataset.audit},
        gate_passed=False,
...
__all__ = ["resolve_carry_facts", "run_bybit_carry_backtest"]
