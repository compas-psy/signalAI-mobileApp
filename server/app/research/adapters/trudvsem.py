...
def _decimal(raw: object) -> Decimal | None:
    if raw is None:
        return None
    text = str(raw).strip().replace("\xa0", "").replace(" ", "").replace(",", ".")
...
def _rows(payload: object) -> list[dict]:
    ...
    if isinstance(results, dict):
        raw = results.get("vacancies")
    else:
        raw = payload.get("vacancies")
...
def parse(fetched: Fetched) -> list[VacancyDatum]:
    ...
    for wrapper in _rows(payload):
        ...
        address = _first_address(vacancy)
        address_region = _mapping(address.get("region"))

        vacancy_id = _text(vacancy, "id", "vacancy-id", "vacancy_id")
        title = _text(vacancy, "job-name", "job_name", "name", "profession")
        employer_code = _text(
            company, "companycode", "company-code", "company_code", "id"
        ) or _text(vacancy, "companycode", "company-code", "company_code")
        employer_inn = _text(company, "inn", "company-inn", "company_inn") or _text(
            vacancy, "inn", "company-inn", "company_inn"
        )
        employer_ogrn = _text(company, "ogrn", "company-ogrn", "company_ogrn") or _text(
            vacancy, "ogrn", "company-ogrn", "company_ogrn"
        )
        ...
        result.append(
            VacancyDatum(
                ...
                employer_name=_text(company, "name", "company-name", "company_name")
                or _text(vacancy, "company-name", "company_name"),
                title=title,
                region_code=_text(region, "region_code", "region-code", "code")
                or _text(address_region, "region_code", "region-code", "code")
                or _text(vacancy, "region_code", "region-code"),
                region_name=_text(region, "name", "region_name", "region-name")
                or _text(address_region, "name", "region_name", "region-name"),
                published_at=_moment(
                    vacancy.get("creation-date")
                    or vacancy.get("creation_date")
                    or vacancy.get("published_at")
                    or vacancy.get("date")
                ),
                modified_at=_moment(
                    vacancy.get("date-modified")
                    or vacancy.get("date_modified")
                    or vacancy.get("modified")
                    or vacancy.get("modified_at")
                    or vacancy.get("updated_at")
                    or vacancy.get("change-date")
                ),
                salary_min=_decimal(
                    vacancy.get("salary_min")
                    if vacancy.get("salary_min") is not None
                    else vacancy.get("salary-min")
                ),
                salary_max=_decimal(
                    vacancy.get("salary_max")
                    if vacancy.get("salary_max") is not None
                    else vacancy.get("salary-max")
                ),
...
def page_url(*, offset: int = 0, limit: int = PAGE_LIMIT, modified_from: str = "") -> str:
    query: dict[str, object] = {
        ...
        "limit": min(PAGE_LIMIT, max(1, int(limit))),
    }
    if modified_from:
...
def remaining_pages(
    fetched: Fetched, *, cursor: Cursor | None = None, limit: int = PAGE_LIMIT
) -> list[RemoteObject]:
    ...
    return [
        RemoteObject(
            url=page_url(offset=offset, limit=page_size, modified_from=modified_from),
            kind="vacancies",
        )
