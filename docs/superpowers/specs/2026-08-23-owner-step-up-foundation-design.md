...
- `ensureOwnerStepUpKey` → creates/reuses a P-256 Keystore key and returns the SPKI public key
- `signOwnerStepUpMessage` → invokes `BiometricPrompt` with a `Signature` backed by the key and signs exactly the server-provided UTF-8 message
