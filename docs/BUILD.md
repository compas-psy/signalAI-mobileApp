...
| «Run workflow» в Actions | нет | тот же APK, собранный в CI |
...
Actions → **Android sideload APK** → **Run workflow** → выбрать ветку.
...
```bash
git tag sideload-$(date +%Y%m%d-%H%M) && git push --tags
```
...
Живой режим в браузере не поднимется: он ходит в Keystore и на диск через
нативные каналы, которых там нет. Логика экранов, скоринг и лимиты при этом
настоящие — этим способом за один день нашлись три дефекта, которых не
увидели ни анализатор, ни весь набор тестов.
