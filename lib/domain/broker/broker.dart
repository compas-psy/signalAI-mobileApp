...
enum BrokerId {
  ...
  /// Есть ли у площадки песочница, доступная владельцу.
  ///
  /// У Bybit тестнет как сервис существует, но отдельного доступа к нему у
  /// владельца нет, и заводить его ради проверки механики он не станет.
  /// Значит, для крипты песочницы нет вовсе: сделка там либо виртуальная на
  /// устройстве, либо живая, и промежуточного состояния не бывает.
  ///
  /// Отсюда следует и обратное: требовать прогона в песочнице от площадки,
  /// у которой её нет, — значит закрыть живой режим навсегда. Требование
  /// адресуется только Т-Инвестициям, где песочница настоящая и бесплатная.
  bool get hasSandbox => this == BrokerId.tinvest;
