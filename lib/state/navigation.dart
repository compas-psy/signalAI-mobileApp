...
///
/// Тона не было вовсе: галочка и зелёный цвет стояли в разметке жёстко, а
/// через тот же тост шли «Проверка не пропустила», «Вход стоп-заявкой ещё
/// не реализован» и любая ошибка пути к бирже. Отказ выглядел ровно как
/// успех — приложение поздравляло владельца с тем, что заявка не ушла.
enum ToastTone {
  /// Действие состоялось.
  success,

  /// Действие состоялось не полностью либо требует внимания.
  warning,
...
///
/// Отдельно от [RiskMode] намеренно. `CAUTION` означает «допуск к живым
/// деньгам урезан» — утверждение про риск, а не про источники, и метить им
/// молчащую биржу значит врать в обе стороны: владелец видит ограничение
/// риска там, где его нет, и не видит причины, по которой пропали идеи.
///
/// Признаки деградации в приложении уже вычислялись — `lastResultPartial`,
/// неполный `sourceNote`, журнал прогонов, — но не были нарисованы нигде.
/// Экран выглядел одинаково и когда всё посчитано, и когда половина
/// источников молчала.
enum DataHealth {
  /// Все включённые источники ответили, расчёт полный.
  full('данные полные'),

  /// Часть источников молчала: идеи посчитаны, но не по всему рынку.
  partial('данные неполные'),

  /// Расчёта нет вовсе — последний прогон не удался.
  blind('данных нет');
