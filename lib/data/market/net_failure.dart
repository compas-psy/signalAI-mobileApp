...
class MarketDataException implements Exception {
  ...
  /// Что владельцу с этим **делать**. Пустая строка — делать нечего.
  ///
  /// Только действие, без пересказа диагноза. Совет склеивается с
  /// [message] через точку, и когда он начинался с той же мысли, владелец
  /// читал одно и то же дважды: «Соединение с … перехвачено: в цепочке
  /// чужой сертификат. Соединение подменяется в сети: в цепочке
  /// сертификатов чужой. Проверьте VPN…».
  ///
  /// У `dns` совета нет вовсе: сообщение уже называет и причину, и место.
  String get advice => switch (kind) {
        NetFailureKind.geoBlocked =>
          'Данные по ней берёт сервер — приложение обойти это не может.',
